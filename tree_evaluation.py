...
full_preds = []
# probtype = ["exp-L1", "exp-L2","sigmoid","hardtanh","square-like-sigmoid", "L1-prob", "L2-prob"]
...
def metrics_in_batches(model, batch_size, metrics_for_eval, probtype, alpha = 0, ):
    num_instances = data_x.shape[0]
    num_batches = math.ceil(num_instances / batch_size)
    # metrics = {prob:linear.get_metrics(metrics_for_eval, num_classes=data_y.shape[1]) for prob in probtype}
    metrics = {}
    metrics.update({prob: linear.get_metrics(metrics_for_eval, num_classes=data_y.shape[1]) for prob in probtype})
    metrics.update({prob+str(A): linear.get_metrics(metrics_for_eval, num_classes=data_y.shape[1]) for prob in probtype_A for A in A_range})
...
t = time.time()
metrics = metrics_in_batches(model, 1000, metrics_for_eval, probtype_A)
...
for key in metrics_for_eval:
    ...
    t = time.time()
    key_metric = [key]
    metrics = metrics_in_batches(model, 1000, key_metric, probtype_A, best_A)
    print(f"predicition time {time.time()-t:.2f} sec")
    for prob in probtype_A:
...
best_A = float(max(re_organized_score, key=re_organized_score.get))
t = time.time()
metrics = metrics_in_batches(model, 1000, metrics_for_eval, probtype, best_A)
print(f"predicition time {time.time()-t:.2f} sec")
for prob in probtype:
    eval = metrics[prob].compute()
    if prob in probtype_A:  
