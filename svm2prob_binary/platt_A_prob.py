...
from platt_scaling import sigmoid_train_A, sigmoid_predict_A


def decision_value_to_prob(decision_values, A):
    ...
    prob = np.expand_dims(np.array([sigmoid_predict_A(float(x), A) for x in decision_values.squeeze(-1)]), axis=-1)
    return np.where(prob == 1, 1.0 - eps, prob)


def metrics_in_batches(model, batch_size, datasets, positive_label_idx, A):
    ...
    for i in range(num_batches):
        ...
        probs = decision_value_to_prob(preds, A)
